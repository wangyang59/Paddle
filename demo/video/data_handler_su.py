...
class DataHandler(object):
    ...
    def __init__(self, data_pb):
        ...
        assert self.num_colors_ * self.image_size_y_ * self.image_size_x_ == self.data_.shape[
            1]

        self.x_slack_ = self.image_size_x_ - self.patch_size_x_
        self.y_slack_ = self.image_size_y_ - self.patch_size_y_

        video_boundaries, num_frames = self.GetBoundaries(
            data_pb.num_frames_file)
        ...
        self.Reset()
        self.batch_data_ = np.zeros(
            (self.batch_size_, self.seq_length_ * self.frame_size_),
            dtype=np.float32)
    ...
    def Crop(self, data, num_crops=1):
        d = data.reshape((data.shape[0], self.num_colors_, self.image_size_y_,
                          self.image_size_x_))
        if self.x_slack_ > 0:
        ...
        crops = np.zeros((num_crops, data.shape[0], self.num_colors_,
                          self.patch_size_y_, self.patch_size_x_))
        seq_length = data.shape[0]
        for i in xrange(num_crops):
            crops[i, :, :, :, :] = d[:, :, y_offset[i]:y_offset[
                i] + self.patch_size_y_, x_offset[i]:x_offset[i] +
                                     self.patch_size_x_]
    ...
    def GetResults(self, predictions):
        ...
        for i in xrange(self.num_videos_):
            end = start + 1 + max(0, (self.num_frames_[i] - self.seq_length_) /
                                  self.seq_stride_)
            correct += (predictions[start:end, :].argmax(axis=1) ==
                        self.labels_[i]).sum()
            pooled_pred = predictions[start:end, :].mean(axis=0)
            pooled_correct += pooled_pred.argmax() == self.labels_[i]
            start = end
        return correct / float(self.dataset_size_), pooled_correct / float(
            self.num_videos_)
    ...
    def DisplayData(self,
        ...
        if self.num_colors_ == 3:
            d = data[0, :].reshape(self.seq_length_, self.num_colors_,
                                   self.patch_size_y_, self.patch_size_x_)
            r = rec[0, :].reshape(-1, self.num_colors_, self.patch_size_y_,
                                  self.patch_size_x_)
            f = fut[0, :].reshape(-1, self.num_colors_, self.patch_size_y_,
                                  self.patch_size_x_)
            im1 = np.zeros(
                (self.patch_size_y_, self.patch_size_x_, self.num_colors_),
                dtype=np.uint8)
            im2 = np.zeros(
                (self.patch_size_y_, self.patch_size_x_, self.num_colors_),
                dtype=np.uint8)
            ...
            for i in xrange(self.seq_length_):
                for j in xrange(self.num_colors_):
                    im1[:, :, j] = (
                        (d[i, j, :, :] * self.std_[j]) + self.mean_[j]
                    ).astype(np.uint8)
...
class UnlabelledDataHandler(object):
    ...
    def GetBatch(self, verbose=False):
        ...
        for j in xrange(batch_size):
            ...
            self.batch_data_[j, :] = f[self.dataset_name_][start:
                                                           end, :].reshape(-1)
            f.close()
...
class VideoPatchDataHandler(object):
    ...
    def DisplayData(self,
        ...
        if self.is_color_:
            ...
            data = data[case_id, :].reshape(-1, self.image_size_,
                                            self.image_size_)
        ...
        for i in xrange(self.seq_length_):
            ...
            if self.is_color_:
                ...
                plt.imshow(
                    data[i, :, :], cmap=plt.cm.gray, interpolation="nearest")
            plt.axis('off')
        plt.draw()
        if output_file1 is not None:
            print output_file1
